...
function (_React$Component) {
  ...
  _proto.render = function render() {
    var _this$props = this.props,
        ...
        props = (0, _objectWithoutPropertiesLoose2.default)(_this$props, ["style", "className", "event", "selected", "isAllDay", "onSelect", "onDoubleClick", "localizer", "continuesPrior", "continuesAfter", "accessors", "getters", "children", "renderContent", "employees", "components"]);
    var title = accessors.title(event); // let tooltip = accessors.tooltip(event)

    var end = accessors.end(event);
    var start = accessors.start(event);
    var allDay = accessors.allDay(event);
    var employeeId = event.EmployeeID; // format title HH:MM - HH:MM : title

    var startTime = (0, _moment.default)(start).format('hh:mm A');
    var endTime = (0, _moment.default)(end).format('hh:mm A');
    var titleFormmat = startTime + " - " + endTime + ": " + title;

    if (renderContent !== undefined && renderContent === 'roster') {
      var employee = _lodash.default.find(employees, {
        EmployeeID: employeeId
      });

      if (employee !== undefined) {
        // FirstName.LastName: HH:MM AM:PM - HH:MM AM:PM
        titleFormmat = employee.EmployeeFirstName + "." + _lodash.default.first(employee.EmployeeLastName) + "\uFF1A" + startTime + " - " + endTime;
      }
    }
