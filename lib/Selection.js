...
var Selection =
  ...
  (function() {
    ...
    _proto._addLongPressListener = function _addLongPressListener(
      ...
      var touchEndListener = null // const handleTouchStart = initialEvent => {
      //   timer = setTimeout(() => {
      //     cleanup()
    ...
    _proto._addInitialEventListener = function _addInitialEventListener() {
      ...
      var mouseDownListener = addEventListener('mousedown', function(e) {
        _this._onInitialEventListener.remove()

        _this._handleInitialEvent(e)

        _this._onInitialEventListener = addEventListener(
          'mousedown',
          _this._handleInitialEvent
        )
      })
      var touchStartListener = addEventListener('touchstart', function(e) {
        _this._onInitialEventListener.remove()

        _this._onInitialEventListener = _this._addLongPressListener(
          _this._handleInitialEvent,
          e
        )
      })
